    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        //数字をintへ返還
            ...
            if ((namelength <= 0)||(namelength >= 21)||(age <= 0)||(age >=150)) {
            	...
            	// 取り出した情報を updateBean に格納する
            	updateBean.setName(name);
            	updateBean.setAge(age);
            	updateBean.setBirthyear(birthyear);
            	updateBean.setBirthmonth(birthmonth);
            	updateBean.setBirthday(birthday);

            	// モデルをインスタンス化する
            	UpdateModel updateModel = new UpdateModel();

            	//会員情報の登録
            	if ((updateModel.updateUserDate(memberno,name,age,birthyear,birthmonth,birthday))&&(memberno != "" )) {
